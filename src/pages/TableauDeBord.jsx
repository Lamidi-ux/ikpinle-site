...
function SectionProducteurs() {
  ...
  const [form, setForm] = useState({ nom: "", telephone: "", commune: "", culture: "mais", superficieHa: "", typeSol: "", whatsapp: false });
  ...
  const soumettre = async (e) => {
    ...
    setErreur("");
    try {
      await appeler("/api/producteurs", { method: "POST", body: JSON.stringify(form) });
      setForm({ nom: "", telephone: "", commune: "", culture: "mais", superficieHa: "", typeSol: "", whatsapp: false });
      charger();
    } catch (e) {
      setErreur(e.message);
    } finally {
  ...
  return (
    <div className="grid md:grid-cols-[1fr_1.2fr] gap-8">
      <form onSubmit={soumettre} className="bg-[#FBF7EE] border border-[#D9C9A8] rounded-2xl p-6 space-y-3 h-fit">
        ...
        <input placeholder="Téléphone (+229...)" required value={form.telephone} onChange={(e) => setForm({ ...form, telephone: e.target.value })}
          className="w-full bg-transparent border border-[#D9C9A8] rounded-sm px-3 py-2 text-sm focus:outline-none focus:border-[#2E6B8A]" />
        <input placeholder="Commune" required value={form.commune} onChange={(e) => setForm({ ...form, commune: e.target.value })}
          className="w-full bg-transparent border border-[#D9C9A8] rounded-sm px-3 py-2 text-sm focus:outline-none focus:border-[#2E6B8A]" />
        <select value={form.culture} onChange={(e) => setForm({ ...form, culture: e.target.value })}
          className="w-full bg-transparent border border-[#D9C9A8] rounded-sm px-3 py-2 text-sm focus:outline-none focus:border-[#2E6B8A]">
          {CULTURES.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <input placeholder="Superficie (ha, optionnel)" value={form.superficieHa} onChange={(e) => setForm({ ...form, superficieHa: e.target.value })}
          className="w-full bg-transparent border border-[#D9C9A8] rounded-sm px-3 py-2 text-sm focus:outline-none focus:border-[#2E6B8A]" />
      ...
      <div>
        <h3 className="font-serif text-lg mb-3">Producteurs enregistrés ({producteurs.length})</h3>
        {chargement ? (
          ...
          <div className="border border-[#D9C9A8] rounded-2xl overflow-hidden max-h-96 overflow-y-auto">
            {producteurs.map((p) => (
              <div key={p.id} className="px-4 py-3 border-b border-[#D9C9A8] last:border-0 bg-[#FBF7EE] flex justify-between items-center text-sm">
                <div>
                  <div className="font-medium">{p.nom || p.telephone}</div>
                  <div className="text-[#7A6B4F] text-xs">{p.commune} · {p.culture}</div>
                </div>
                <span className="font-mono text-[10px] uppercase text-[#7A6B4F]">{p.whatsapp ? "WhatsApp" : "SMS"}</span>
              </div>
            ))}
            {producteurs.length === 0 && (
              <p className="px-4 py-6 text-sm text-[#7A6B4F] text-center">Aucun producteur enregistré ici pour l'instant.</p>
            )}
          </div>
        )}
      </div>
...
function SectionDetectionIA() {
  ...
  const lancer = async (e) => {
    ...
    setResultat(null);
    try {
      await appeler("/api/ia/detecter-saison", { method: "POST", body: JSON.stringify({ commune, culture }) });
      const recommandation = await appeler(`/api/ia/recommandation?commune=${encodeURIComponent(commune)}&culture=${culture}`);
  ...
  return (
    <div className="bg-[#FBF7EE] border border-[#D9C9A8] rounded-2xl p-6">
      ...
      <form onSubmit={lancer} className="flex flex-wrap gap-3 mb-4">
        <input placeholder="Commune" required value={commune} onChange={(e) => setCommune(e.target.value)}
      ...
      {resultat && (
        <div className="border-t border-[#D9C9A8] pt-4 text-sm space-y-2">
          ...
          {resultat.conseilsRendement?.length > 0 && (
            <ul className="list-disc list-inside text-[#4A4033] space-y-1">
              {resultat.conseilsRendement.map((c, i) => <li key={i}>{c}</li>)}
            </ul>
          )}
        </div>
      )}
...
function SectionChatbot() {
  ...
  const poser = async (e) => {
    ...
    setQuestion("");
    try {
      const donnees = await appeler("/api/ia/chatbot", { method: "POST", body: JSON.stringify({ question: q }) });
  ...
  return (
    <div className="bg-[#FBF7EE] border border-[#D9C9A8] rounded-2xl p-6">
      ...
      <div className="space-y-3 max-h-64 overflow-y-auto mb-4">
        ...
        {historique.length === 0 && (
          <p className="text-sm text-[#7A6B4F]">Posez une question sur une culture (cycle, besoin en eau, semis, rendement).</p>
        )}
      </div>
...
export default function TableauDeBord() {
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-12 space-y-10">
      <div>
        <div className="font-mono text-xs uppercase tracking-widest text-[#B8542D] mb-2">Tableau de bord</div>
